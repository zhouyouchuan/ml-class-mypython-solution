...
def cofiCostFunc(params, Y, R, num_users, num_movies, num_features, Lambda):
    ...
    h = np.dot(X,Theta.T)
    minus = R*h - R*Y
    diff_pow = np.power(minus,2)
    ureg_J = (1.0/2) * np.sum(diff_pow)
    J = ureg_J + (1.0*Lambda/2)* np.sum(np.power(X,2)) + (1.0*Lambda/2)*np.sum(np.power(Theta,2))
    ureg_X = np.dot(minus,Theta)
    ureg_theta = np.dot(minus.T,X)
    X_grad = ureg_X + Lambda * X
    Theta_grad = ureg_theta + Lambda * Theta
