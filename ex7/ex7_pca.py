...
import sys  
sys.path.append('C:\\Users\\deeplearner\\MLP')
...
from retainedcal import retainedcal
...
Xs = np.array([X[s] for s in sel.astype('int')])
xs = Xs[:, 0]
ys = Xs[:, 1]
zs = Xs[:, 2]
...
# Plot in 2D
plt.figure()
zs = np.array([Z[s] for s in sel.astype('int')])
idxs = np.array([idx[s] for s in sel.astype('int')])
